...
function get_redis_connection() {
    ...
    if ($redis === null) {
        ...
        // Проверяем наличие ключей и задаем значения по умолчанию
        $redis_enabled = $settings['redis_enabled'] ?? 0;
...
function save_to_redis_cache($cache_key, $data, $path) {
    ...
    if ($redis !== false) {
        ...
        $lifetime = $settings['cache_rules'][$path]['lifetime'] ?? ($settings['default_lifetime'] ?? 3600);
